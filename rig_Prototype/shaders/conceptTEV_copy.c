#pragmalanguage GLSL
#define_nullCOLOR float4(1.0, 1.0, 1.0, 1.0)
#define_nullMAP(color, scalar) ( \
((color) * (scalar)) + \
...
float4x4WorldViewProjXf : WorldViewProjection;
float4x4WorldXf         : World;
float4x4WorldIXf        : WorldInverse;
float4x4WorldITXf       : WorldInverseTranspose;
float4x4ViewIXf         : ViewInverse;	//
float4x4ViewXf          : View;
float4x4ViewITXf        : ViewInverseTranspose;
...
float4cDiffuse  : Diffuse  < string UIWidget = "cDiffuse";  > = { 1.0, 0.75, 0.75, 1.0 };
float3cAmbient  : Ambient  < string UIWidget = "cAmbient";  > = { 0.1, 0.2 , 0.3       };
float3cSpecular : Specular < string UIWidget = "cSpecular"; > = { 0.5, 0.8 , 1.0       };
float3cRim      : Diffuse  < string UIWidget = "cRim";      > = { 1.0, 0.5 , 1.0       };

intspecExp <  int UIMin = 1; int UIMax = 128; 	> = 16;

floatKd    < float UIMin = 0.0;	float UIMax = 1.0; > = 1.0;
floatKa    < float UIMin = 0.0;	float UIMax = 1.0; > = 1.0;
floatKs    < float UIMin = 0.0;	float UIMax = 1.0; > = 0.5;
floatK_rim < float UIMin = 0.0;	float UIMax = 4.0; > = 0.5;

floatK_imgCO 	< float UIMin = 0.0;	float UIMax = 1.0; > = 0.0;	//texture map
floatK_imgDF 	< float UIMin = 0.0;	float UIMax = 1.0; > = 1.0;	//texture map
floatK_imgAM 	< float UIMin = 0.0;	float UIMax = 1.0; > = 1.0;	//texture map
floatK_imgRM 	< float UIMin = 0.0;	float UIMax = 1.0; > = 0.0;	//texture map
...
VS_OUTPUTvs_main_OG(VS_INPUT IN)
{
VS_OUTPUT OUT;
...
VS_OUTPUTvs_main_ZB(VS_INPUT IN)
{
VS_OUTPUT OUT;
...
//////////////////////Pixel Shader(s) ////////////////////////
///////////////////////////////////////////////////////////////
float4ps_main_TEMP(VS_OUTPUT IN): COLOR
{
...
float4ps_main_OG(VS_OUTPUT IN): COLOR
{
float4 lookUpMAP = tex2D(ColorSampler, IN.tc);
...
float4ps_main_TEV(VS_OUTPUT IN): COLOR
{
float diff = IN.vLight.x;	float spec = saturate(IN.vLight.y);	float rim = saturate(IN.vLight.z);
...
float4ps_main_lookUpMap(VS_OUTPUT IN): COLOR
{
float4 lookUpMAP = tex2D(LookUpSampler, IN.tcLookUp.xy);
...
float4ps_main_colorMap(VS_OUTPUT IN): COLOR
{
float4 lookUpMAP = tex2D(ColorSampler, IN.tc);
return lookUpMAP;
}
