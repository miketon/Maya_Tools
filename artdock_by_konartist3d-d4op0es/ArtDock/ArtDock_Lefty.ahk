...
ControlSource = MenuDock_Lefty.txt
...
{	
	...
	; touch up
	Else If (!stat) {
		Run, %A_AhkPath% "%A_ScriptFullPath%" "txt_Lefty\%ControlSource%"
		ExitApp
	}
	Return -stat
}
...
CtlDockSlider_Proc(stat, x, y, cnum)
{
	...
	ControlSource = %ctl%.txt
	ControlSourceP = alt_Lefty\%ctl%P_P.txt
		...
		Else {
			...
			If (Abs(difference) > Ctl%cnum%_Step) {
				If (difference > 50) {
					...
					Run, %A_AhkPath% "%A_ScriptFullPath%\..\alt_Lefty\HelperDock.ahk" "%ControlSourceH%"
					ExitApp
				}
			}
...
CtlAppDock_Proc(stat, x, y, cnum)
{
	...
	Else If (!stat) {
		...
		;WinClose HelperDock.ahk
		Sleep, 50
		Run, %A_AhkPath% "alt_Lefty\AppDock.ahk"  "%ControlSourceH%"
	}
